---
description: Audit a Lore graph in another project. Usage: /lore:probe <path-to-project>
argument-hint: <path-to-project>
allowed-tools: [Bash]
model: haiku
---

Audit the Lore graph of an external project without switching working
directory. The user passes a path as `$ARGUMENTS`.

## Steps

1. **Resolve the DB path**: `<path>/.lore/lore.db`. If the path does not
   exist, stop and report it clearly — do not create anything.

2. **Run two read-only CLI calls**:

   ```bash
   lore audit --db <path>/.lore/lore.db --json
   lore stats --db <path>/.lore/lore.db
   ```
...
- Do not read individual node bodies — this is a structural probe, not an
  ...
  use `/lore:show`.
- If `lore stats` reports no calls, say "no MCP activity recorded" — don't
