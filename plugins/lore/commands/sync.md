---
description: Sync the Lore graph with code changes since a git revision. Detects new files that should become nodes and existing nodes whose source files changed. Confirms before persisting.
allowed-tools: [Bash, Read, Glob, Grep]
argument-hint: [git-rev]
---

Bring the Lore graph back in line with the code. Reads `git diff` from
...
direction from `/lore:reconcile`: reconcile detects rotten nodes,
...
2. **Run `lore sync-plan`:**

   ```bash
   lore sync-plan --since <rev>
   ```
...
4. **Walk unmapped files in batches.** For each repo, group unmapped
   ...
   - If a file clearly maps onto an existing node by name overlap or
     subject, mention that and propose `lore_update_node` instead of
     a new node.
   ...
   persisting. Persist the confirmed batch with `lore_add_nodes(...)`
...
5. **Walk mapped files.** For each, the JSON gives you the existing
   ...
   - Decide: did the change update the node's behavior? If yes,
     propose a `lore_update_node` with `body=...,
...
6. **Audit at the end.** After all writes, run `lore_audit()` and
...
- If the diff is enormous (>200 unmapped files), suggest a smaller
  scope (`/lore:sync HEAD~3`) before drowning the conversation.
