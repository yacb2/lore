---
description: Initialize Lore for this project — one smart command that handles single repos and multi-repo workspaces, with scan or manual seeding.
allowed-tools: [Bash, Read, Glob, Grep, Agent]
---

Set up Lore for this project in a single command. Replaces the old
...
Call `lore_list(type="module")` to see if modules already exist.

- **If >0 modules** → say "Lore already has N top-level modules for this
  project. Use `/lore:bootstrap` to scan the code and propose
  additional capabilities/flows, or `/lore:audit` to see current
  state." Do not re-seed. Stop.
- **If 0 modules** → continue.

Also ensure the DB file exists: run `lore init --db .lore/lore.db` once
...
> "I see a workspace with N repos: `<list>`. How do you want Lore to
> map this project?
>
...
4. Persist in one transaction:
   - `lore_add_nodes(...)` with every module (repos + inner).
   - `lore_add_edges(...)` with `part_of` from each inner module to
...
`lore_add_nodes` batch. No capabilities/flows inferred. Every module
...
1. Call `lore_audit()`. Read `nodes_total` and `edges_total`.
...
Tell the user:
- Add capabilities and flows as you work (the `lore-usage` skill will
  prompt you).
- Run `/lore:audit` anytime for structural health.
- Run `/lore:reconcile` to see drift between graph and code.
- Run `/lore:bootstrap` later if you want a fresh Sonnet pass over
...
- `model`: the configured exploration model (`.lore/config.json →
