---
description: Re-scan a DomainTome-initialized project and extend the graph with new capabilities and flows. Use AFTER /dt:init, when the codebase has grown or drifted.
allowed-tools: [Bash, Read, Glob, Grep, Agent]
---

Re-scan a project that already has DomainTome initialized, and propose
additional capabilities/flows to extend the graph. Complements
`/dt:init` (which handles first-time setup). Run this when:
...
**For first-time setup, use `/dt:init` instead.** This command
assumes modules already exist and focuses on extending them.

1. **Verify the graph is initialized.** Call `dt_list(type="module")`.
   If there are zero modules, stop and tell the user to run
   `/dt:init` first — bootstrap is not a first-time setup tool.

2. **Delegate discovery to a sub-agent.** Bootstrap is run infrequently
   and the result seeds every future DomainTome operation, so **precision
   ...
   DomainTome — only return a JSON proposal for the caller to review.
...
5. **Mandatory verification — do NOT skip this.** Hallucinating a
   ...
   a. Call `dt_audit()`. The report includes `nodes_total` and
      `edges_total`.
   ...
   c. Report exactly one of two outcomes:
      - **Match**: "Sembrados N_nodes nodos y N_edges edges. Verificado
        con `dt_audit`." Then surface any audit findings.
...
6. **Create `.dt/config.json`** if it doesn't exist, seeded with the
...
reminder that the user should review `inferred_from_code` nodes with `dt
stats` and `dt audit --provenance` (planned) as they work on the project.
