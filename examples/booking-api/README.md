# booking-api — Lore demo project

A tiny FastAPI booking system used to demo Lore in ~60 seconds.
...
a reservation be cancelled?"* requires real exploration without Lore.
...
Without Lore, an assistant has to grep, read 4+ files, and reconstruct
this list. With Lore, it is a single `lore_query` returning typed nodes
...
```bash
# Without Lore: bytes the assistant must read
wc -c app/api.py app/services.py app/rules.py app/models.py scripts/expire_pending.py
# => 6,213 bytes

# With Lore: bytes returned by a single query
lore init                                  # creates .lore/lore.db
sqlite3 .lore/lore.db < scripts/seed_graph.sql
lore query cancel | wc -c
# => 1,485 bytes
```

`scripts/seed_graph.sql` is the source of truth for the demo graph (12 nodes, 15 edges covering 4 cancellation paths, 3 rules, 2 entities). `.lore/` is gitignored so the DB is rebuilt locally.
